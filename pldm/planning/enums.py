...
class PooledMPCResult:
    ...
    planning_counts: dict = field(default_factory=dict)
    ...
    def concatenate_chunks(self):
        ...
        if self.extra:
            keys = set().union(*(counts.keys() for counts in self.extra))
            self.planning_counts = {
                key: sum(counts.get(key, 0) for counts in self.extra) for key in keys
            }
        else:
            self.planning_counts = {}
