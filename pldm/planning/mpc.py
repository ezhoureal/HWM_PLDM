...
from pldm.planning.distillation import (
    DistillationTraceCollector,
...
class MPCEvaluator(ABC):
    def __init__(
        ...
        self.distill_policy_controller = self._load_distill_policy_controller()
        self.distill_trace_collector = self._make_distill_trace_collector()

    def _load_distill_policy_controller(self):
        policy_path = getattr(self.config, "distill_policy_path", None)
        if not policy_path:
            return None

        return DistilledPolicyController.from_checkpoint(
            path=policy_path,
            device=self.device,
            mode_override=getattr(self.config, "distill_policy_mode", None),
            max_action_abs=getattr(self.config, "distill_policy_max_action_abs", None),
        )

    def _make_distill_trace_collector(self):
        trace_dir = getattr(self.config, "distill_trace_dir", None)
        if not trace_dir:
            return None

        return DistillationTraceCollector(
            out_dir=trace_dir,
            shard_size=getattr(self.config, "distill_trace_shard_size", 10000),
            prefix=getattr(self.config, "distill_trace_prefix", "hwm_low_level"),
        )
    ...
    def _construct_h_planner(self, n_envs: int):
        ...
        h_planner = TwoLvlPlanner(
            ...
            policy_controller=self.distill_policy_controller,
        )
    ...
    def _perform_mpc_in_chunks(self):
        ...
        for chunk_size in chunk_sizes:
            ...
            mpc_data.extra.append(mpc_result.planning_counts or {})
        ...
        if self.distill_trace_collector is not None:
            self.distill_trace_collector.close()

        return mpc_data

    def _perform_h_mpc(self, h_planner, planner, envs, env_offset: int = 0):
        """
        ...
        if self.config.final_trans_steps:
            ...
            mpc_result = MPCResult(
                ...
                planning_counts=self._sum_planning_counts(
                    mpc_result_1.planning_counts,
                    mpc_result_2.planning_counts,
                ),
            )
        else:
    ...
    def _sum_planning_counts(self, *counts):
        out = {}
    ...
    def _perform_mpc(
        ...
        targets_t = self._encode_targets(
            envs, bilevel_planning=bilevel_planning, squeeze_target=False
        )
        low_policy_active = (
            bilevel_planning
            and getattr(planner, "uses_distilled_policy", False)
        )
        l1_targets_t = None
        if bilevel_planning and (
            self.distill_trace_collector is not None or low_policy_active
        ):
            l1_targets_t = self._encode_targets(
                envs, bilevel_planning=False, squeeze_target=True
            )
        planner.reset_targets(targets_t, repr_input=True)
        if low_policy_active and hasattr(planner, "set_policy_goal"):
            planner.set_policy_goal(l1_targets_t)
        ...
        collect_teacher_traces = (
            bilevel_planning
            and self.distill_trace_collector is not None
            and not low_policy_active
        )

        for i in tqdm(range(max_steps), desc="Planning steps"):
            need_model_inputs = (
                i % self.config.replan_every == 0
                or low_policy_active
                or collect_teacher_traces
            )
            if need_model_inputs:
                curr_proprio_pos, curr_proprio_vel, curr_locations = (
                    self._get_current_model_inputs(envs)
                )
            else:
            ...
            if i % self.config.replan_every == 0:
                ...
                self._accumulate_planning_counts(planning_counts, planner)
            elif low_policy_active:
                planning_result = planner.act_low_level(
                    obs_t,
                    curr_proprio_pos=curr_proprio_pos,
                    curr_proprio_vel=curr_proprio_vel,
                    curr_locations=curr_locations,
                    plan_size=self.model.config.step_skip,
                )
                self._accumulate_planning_counts(planning_counts, planner)
            ...
            if low_policy_active:
                planned_actions = planning_result.actions.detach().cpu()
            else:
                planned_actions = (
            ...
            if collect_teacher_traces:
                self._record_distill_trace(
                    planner=planner,
                    obs_t=obs_t,
                    curr_proprio_pos=curr_proprio_pos,
                    curr_proprio_vel=curr_proprio_vel,
                    curr_locations=curr_locations,
                    actions=planned_actions[:, 0],
                    z_g=l1_targets_t,
                    targets=targets,
                    rewards=rewards_t,
                    infos=infos,
    ...
    def _get_current_model_inputs(self, envs):
        if self.model.level1.using_proprio_pos:
            curr_proprio_pos = [e.get_proprio_pos(normalized=True) for e in envs]
            curr_proprio_pos = torch.from_numpy(np.stack(curr_proprio_pos)).float()
        else:
            curr_proprio_pos = None

        if self.model.level1.using_proprio_vel:
            curr_proprio_vel = [e.get_proprio_vel(normalized=True) for e in envs]
            curr_proprio_vel = torch.from_numpy(np.stack(curr_proprio_vel)).float()
        else:
            curr_proprio_vel = None
        ...
        return curr_proprio_pos, curr_proprio_vel, curr_locations

    def _accumulate_planning_counts(self, planning_counts, planner):
        stats = getattr(planner, "last_call_stats", None)
        if stats:
            for key, value in stats.items():
                planning_counts[key] = planning_counts.get(key, 0) + int(value)
            return

        planning_counts["l1_planner_calls"] = (
            planning_counts.get("l1_planner_calls", 0) + 1
        )

    def _record_distill_trace(
        self,
        planner,
        obs_t,
        curr_proprio_pos,
        ...
        if getattr(planner, "cached_subgoal", None) is None:
            return

        backbone_output = planner._encode_l1(
            current_state=obs_t,
            curr_proprio_pos=curr_proprio_pos,
            curr_proprio_vel=curr_proprio_vel,
            curr_locations=curr_locations,
        ...
        success = torch.tensor(
            [
                float(info.get("success", rewards[idx].item()))
                for idx, info in enumerate(infos)
            ]
        )
        self.distill_trace_collector.record_batch(
            z_t=backbone_output.encodings,
            z_subgoal=planner.cached_subgoal,
            action=actions,
            z_g=z_g,
            target=targets,
            reward=rewards,
